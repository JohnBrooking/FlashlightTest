...
- (void)barcodeEnterByHand {
    [self barcodeScanDone];
    [self.plugin returnSuccess:@"ENTER" format:@"" cancelled:TRUE callback:self.callback];
}
...
//--------------------------------------------------------------------------
- (IBAction)tapToEnterPressed:(id)sender {
    [self.processor performSelector:@selector(barcodeEnterByHand) withObject:nil afterDelay:0];
}
//--------------------------------------------------------------------------
...
- (UIView*)buildOverlayView {
    ...
    //Check for IOS7 and its global status bar
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
        bounds = CGRectMake(0, 20, bounds.size.width, bounds.size.height - 20);
    }
    ...
    UIToolbar* topToolbar = [[[UIToolbar alloc] init] autorelease];
    topToolbar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleBottomMargin;
    
    id backButton = [[[UIBarButtonItem alloc] autorelease]
                     initWithTitle:@"< Tap to Enter Code" style:UIBarButtonItemStylePlain
                     target:(id)self
                     action:@selector(tapToEnterPressed:)
                     ];
    topToolbar.items = [NSArray arrayWithObjects:backButton, nil];
    ...
    CGFloat topToolbarHeight  = [topToolbar frame].size.height;
    CGRect  topRectArea       = CGRectMake(0, 0, rootViewWidth, topToolbarHeight);
    
    [topToolbar setFrame:topRectArea];
    
    [overlayView addSubview: topToolbar];
